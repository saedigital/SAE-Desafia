    ...
    <table>
      ...
      <tbody>
        <tr v-for="(item, key) in espetaculos" :key="key">
          ...
          <td>{{ item.date }}</td>
          <td>[<router-link :to="`/reservas/${item.id}`">Lista de reservas</router-link>] [<router-link :to="`/nova-reserva/${item.id}`">Fazer reserva</router-link>] [<router-link :to="`/editar-espetaculo/${item.id}`">Editar espetaculo</router-link>] [<a href="#" @click="remove(item.id)">Excluir espetáculo</a>]</td>
        </tr>
      </tbody>
    </table>
  </div>
